...
d3.linegraph = function(noTicks, noDots, parties, partyColors, partyNames, dataMax, dataMin, additionalMonths) {
    /* params */
    if (!parties) {
        parties = ['klp', 'ksp', 'egk', 'pn', 'lp', 'clp', 'snp', 'other'];
    }
    if (!partyColors) {
        partyColors = { 'egk': '#30B24A', 'klp': '#FF5E5E', 'ksp': '#A33E7E', 'lp': '#FFE16B', 'clp': '#89A4DD', 'snp': '#26473E', 'pn': '#5DCCAC', 'other': '#a0a0a0'};
    }
    if (!partyNames) {
        partyNames = {'egk': 'EGK', 'klp': 'KLP', 'ksp': 'KSP', 'lp': 'LP', 'clp': 'CLP', 'snp': 'SNP', 'pn': 'PN', 'other': 'Others'};
    }
    if (!additionalMonths) {
        additionalMonths = 10;
    }
